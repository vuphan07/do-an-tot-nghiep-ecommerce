...
const orderCtrl = {
  getPaymentsAll: async (req, res) => {
    try {
      ...
      const features1 = new APIfeatures(Payments.find(), req.query)
        ...
        .sorting()
        .paginating();
      Promise.all([await features.query.count(), await features1.query]).then(
        (results) => {
          res.json({
            status: "success",
            data: {
  ...
  getPayments: async (req, res) => {
    try {
      const { userId } = req;
      const features = new APIfeatures(Payments.find({ user_id: userId }), {
        ...req.query,
      })
        .filtering()
        .sorting();
  ...
  createPayment: async (req, res) => {
    try {
      const user = await Users.findById(req.userId).select("_id name email");
      ...
      Promise.all([
        ...products.map(({ productId, quantity }) => sold(productId, quantity)),
  ...
  updateOrder: async (req, res) => {
    try {
      const { name, email, products, paymentID, address, amount } = req.body;
